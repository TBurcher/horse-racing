...
function Horse(id, name, x, y){
	...
	this.arrive = function(){
		...
		var tdsName = document.querySelectorAll('#results .name');//Get all table cell to display the result
		...
		tdsName[results.length].innerHTML = this.name;//The class of result look like: result horse1...
...
var num_lap = 1, results = [], resultsName = [], funds = 100, bethorse, amount;

//Start the function when the document loaded
document.addEventListener("DOMContentLoaded", function(event) {

	var horse1 = new Horse('horse1', 'Arno', 15, 4);
	var horse2 = new Horse('horse2', 'Bruno', 15, 8);
	var horse3 = new Horse('horse3', 'Jurian', 15, 12);
	var horse4 = new Horse('horse4', 'Sarah', 15, 16);
	var horse5 = new Horse('horse5', 'Tarik', 15, 20);
	...
	document.getElementById('start').onclick = function(){
		...
		if (funds < amount){
			...
			var tdsName = document.querySelectorAll('#results .name');//Get all cells of result table.
			for (var i = 0; i < tds.length; i++) {
				tds[i].className = 'result';//Reset the result.
				tdsName[i].innerHTML = '';//Reset the result.
			}
